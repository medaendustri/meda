...
import { Search, Filter, Eye, Grid, List, Star, Anchor, Loader2 } from "lucide-react"
      ...
      <section className="relative py-16 bg-gradient-to-br from-[#d84948] via-[#dc5a59] to-[#c73e3d] overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center text-white">
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-4 leading-tight">Dragon Winch Ürünleri</h1>
            <p className="text-lg text-red-100 max-w-3xl mx-auto leading-relaxed">
              Dragon Winch çekme vinci tamburlarının geniş ürün portföyünü keşfedin. Denizcilik, endüstri ve liman
              sektörleri için profesyonel vinç çözümleri.
            </p>
          </div>
        </div>
      </section>
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center space-x-2">
                        ...
                        <span
                          className={`font-medium text-sm ${
                            selectedCategory === category.id ? "text-[#d84948]" : "text-gray-900"
                          }`}
                        >
                          {category.name}
                        </span>
                      ...
                      <Badge
                        ...
                        className={`text-xs ${selectedCategory === category.id ? "bg-[#d84948]" : ""}`}
                      >
                        {category.count}
                      </Badge>
                    </div>
          ...
          <div className="lg:col-span-3">
                    ...
                    <div className="flex border border-gray-200 rounded-lg overflow-hidden">
                      <button
                        ...
                        className={`p-2 transition-colors ${
                          viewMode === "grid" ? "bg-[#d84948] text-white" : "bg-white text-gray-600 hover:bg-gray-50"
                        }`}
                      >
                      ...
                      <button
                        ...
                        className={`p-2 transition-colors ${
                          viewMode === "list" ? "bg-[#d84948] text-white" : "bg-white text-gray-600 hover:bg-gray-50"
                        }`}
                      >
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <p className="text-gray-600 text-sm">
                  <span className="font-semibold text-[#d84948]">{sortedProducts.length}</span> Dragon Winch ürünü
                  bulundu
                  {selectedCategory !== "all" && (
                    <span className="ml-2">- {allCategories.find((c) => c.id === selectedCategory)?.name}</span>
                  )}
                </p>
              </div>
            </div>
            ...
            {loading && (
              <div className="flex items-center justify-center py-16">
                ...
                <span className="ml-2 text-gray-600">Dragon Winch ürünleri yükleniyor...</span>
              </div>
            )}
            ...
            {!loading && (
              ...
              >
                {sortedProducts.map((product) => (
                  ...
                  >
                    <div
                      className={`relative overflow-hidden ${viewMode === "list" ? "w-48 h-32 flex-shrink-0" : "aspect-[4/3]"}`}
                    >
                      <Image
                        src={
                          product.images[0]?.src || "/placeholder.svg?height=240&width=320&query=dragon winch product"
                        }
                      ...
                      <div className="absolute bottom-2 left-2">
                        <Badge
                          className={`text-xs ${product.stock_status === "instock" ? "bg-green-600" : "bg-red-600"}`}
                        >
                          {product.stock_status === "instock" ? "Stokta" : "Stokta Yok"}
                        </Badge>
                      </div>
                      <div className="absolute top-2 right-2">
                        <Badge className="bg-[#d84948] text-white text-xs">Dragon Winch</Badge>
                      </div>
                    </div>

                    <div className={`flex-1 ${viewMode === "list" ? "p-4" : ""}`}>
                      <CardHeader className={`${viewMode === "list" ? "p-0 pb-2" : "p-4 pb-2"}`}>
                        <div className="flex items-center justify-between mb-2">
                          <Badge variant="outline" className="text-[#d84948] border-[#d84948] text-xs">
                        ...
                        <div
                          ...
                          dangerouslySetInnerHTML={{
                            __html: product.short_description || product.description.substring(0, 100) + "...",
                          }}
                        />
                      </CardHeader>

                      <CardContent className={`${viewMode === "list" ? "p-0" : "p-4 pt-0"}`}>
            ...
            {!loading && sortedProducts.length === 0 && (
              <Card className="text-center py-12 border-0 shadow-lg bg-white">
                <CardContent>
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">Dragon Winch Ürünü Bulunamadı</h3>
                  <p className="text-gray-500 mb-4">Aradığınız kriterlere uygun Dragon Winch ürünü bulunamadı.</p>
                  <Button
                    onClick={() => {
